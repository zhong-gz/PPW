...
folder_path = 'StoreDemand/result/'

df=pd.read_csv('StoreDemand/sales_data.csv', encoding='ascii', delimiter=',')
feature_cols = ['Inventory Level', 'Units Sold', 'Units Ordered', 'Price', 'Discount', 'Promotion', 'Competitor Pricing', 'Epidemic']
X = df[feature_cols].values
y = df['Demand'].values

# Normalize the data
scaler = MinMaxScaler()
y = scaler.fit_transform(y.reshape(-1,1))*1
