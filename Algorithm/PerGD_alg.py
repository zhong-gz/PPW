...
class PerGD:
    ...
    def train(self,X,y_ture):
        ...
        d = X_b.shape[1]
        self.coef_ = 2 * np.random.rand(d) - 1
        if len(self.thetas) == 0:
            self.thetas.append(self.coef_.copy())
        Y = np.copy(y_ture)
        if len(self.thetas) < 2:
            # model = Ridge(alpha = 0, fit_intercept=False)
            # model.fit(X_b, Y)
            # self.coef_ = model.coef_.T #+ np.random.normal(0, 0.05, size=model.coef_.T.shape)
            self.coef_ = np.random.randn(X_b.shape[1], 1) # 随机初始化参数，确保是列向量
        else:
            if len(self.thetas) < 2:
                self.means.append(approx_f(X_b, Y))
                grad = grad1(X_b, Y, self.coef_)

                self.coef_ = clip(self.coef_ - self.lr * grad).copy()
                # self.coef_ = (self.coef_ - self.lr * grad).copy()
                # self.history.append(self.coef_.copy())
                self.thetas.append(self.coef_.copy())
            else:
                self.means.append(approx_f(X_b, Y))
                g2, grad_f = grad2(X_b, Y, self.means, self.thetas, self.s1)
                grad = grad1(X_b, Y, self.coef_) + g2
    ...
    def predict(self,X):
        X_b = np.c_[np.ones((X.shape[0], 1)), X]
        predictions = np.dot(X_b, self.coef_)
        return predictions